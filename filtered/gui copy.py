...
class BookRecommendationGUI:
    ...
    def setup_filter_tab(self):
        ...
        self.genre_listbox = tk.Listbox(self.filter_frame, selectmode=tk.MULTIPLE, height=4)
        ...
        self.author_listbox = tk.Listbox(self.filter_frame, selectmode=tk.MULTIPLE, height=4)
        ...
        # Издательство
        ttk.Label(self.filter_frame, text="Издательство:").grid(row=4, column=0, sticky='w', padx=5, pady=5)
        self.publisher_var = tk.StringVar(value="Все")
        publishers = ["Все"] + sorted(self.df['publisher'].unique().tolist())
        self.publisher_combo = ttk.Combobox(self.filter_frame, textvariable=self.publisher_var, values=publishers, state="readonly")
        self.publisher_combo.grid(row=4, column=1, sticky='ew', padx=5, pady=5)
        ...
        self.filter_frame.rowconfigure(6, weight=1)
    ...
    def setup_recommendation_tab(self):
        ...
        self.likes_listbox = tk.Listbox(left_frame, selectmode=tk.MULTIPLE, height=10)
        ...
        self.dislikes_listbox = tk.Listbox(left_frame, selectmode=tk.MULTIPLE, height=10)
        ...
        self.penalty_var = tk.DoubleVar(value=0.7)
    ...
    def apply_filters(self):
        try:
            ...
            if self.publisher_var.get() != "Все":
                filtered_df = filtered_df[filtered_df['publisher'] == self.publisher_var.get()]
    ...
    def get_recommendations(self):
        ...
        try:
            ...
            # Создаем метрики для отфильтрованного датасета (рекомендации из отфильтрованного)
            self.metrics = BookDistanceMetrics(self.filtered_df)
            
            # Получаем рекомендации из отфильтрованного датасета
            recommendations = recommend_based_on_multiple_likes(
                self.metrics,
                liked_indices_full,  # Передаем индексы из полного датасета
                n_recommendations=self.n_rec_var.get(),
                strategy=self.strategy_var.get(),
                disliked_book_indices=disliked_indices_full,  # Передаем индексы из полного датасета
    ...
    def _convert_indices_full_to_filtered(self, full_indices):
        """Конвертирует индексы из полного датасета в индексы отфильтрованного"""
        filtered_indices = []
        
        for full_idx in full_indices:
            # Получаем книгу из полного датасета по индексу
            full_book = self.df.iloc[full_idx]
            
            # Ищем эту книгу в отфильтрованном датасете
            mask = (
                (self.filtered_df['title'] == full_book['title']) & 
                (self.filtered_df['author'] == full_book['author'])
            )
            matching_indices = self.filtered_df[mask].index.tolist()
            
            if matching_indices:
                filtered_indices.append(matching_indices[0])
    ...
    def display_recommendations(self, recommendations, liked_indices_full, disliked_indices_full):
        ...
        for i, (book_idx, similarity) in enumerate(recommendations, 1):
            ...
            for liked_idx in liked_indices_full:
                sim = self.metrics.similarity_score(book_idx, liked_idx)
...
# Функции рекомендательной системы (остаются без изменений)
...
def recommend_based_on_multiple_likes(metrics, liked_book_indices, n_recommendations=10, weights=None, 
    ...
    if verbose:
        _display_recommendations(metrics, recommendations, liked_book_indices, disliked_book_indices)
...
def _combined_strategy_all_books(metrics, liked_indices, weights, exclude_liked):
    """Комбинированная стратегия для ВСЕХ книг"""
    book_scores = {}
...
def _average_strategy_all_books(metrics, liked_indices, weights, exclude_liked):
    """Стратегия усреднения для ВСЕХ книг"""
    book_scores = {}
...
def _union_strategy_all_books(metrics, liked_indices, weights, exclude_liked):
    """Стратегия объединения для ВСЕХ книг"""
    max_scores = {}
...
def _content_boost_strategy_all_books(metrics, liked_indices, weights, exclude_liked):
    """Стратегия усиления контента для ВСЕХ книг"""
...
def _display_recommendations(metrics, recommendations, liked_indices, disliked_indices=None):
    ...
    for i, (book_idx, similarity) in enumerate(recommendations, 1):
        ...
        best_matches = []
        for liked_idx in liked_indices:
            liked_book = metrics.df.iloc[liked_idx]
